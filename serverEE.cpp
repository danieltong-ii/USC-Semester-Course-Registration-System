//
// Created by Daniel Tong on 11/24/22.
// TONGD
// EE450
...
map<string, serverEE> databaseEE;

void populateEEDatabase() {
    ifstream ifs("ee.txt");
    string currline;
    ...
    while (getline(ifs, currline)) {
        ...
        databaseEE.insert(pair<string, serverEE>(v[0], newCourse));
    }
}
...
string processRequest(string course, string category) {
    if (databaseEE.find(course) == databaseEE.end()) {
        return "0";
    }
    else if (databaseEE.find(course) != databaseEE.end()) {
        if (category == "credit") {
            return databaseEE.at(course).getCredits();
        }
        else if (category == "professor") {
            return databaseEE.at(course).getProfessor();
        }
        else if (category == "days") {
            return databaseEE.at(course).getDays();
        }
        else if (category == "course name") {
            return databaseEE.at(course).getCourseName();
        }
    }
    return "0";
}
...
int main(int argc, char *argv[]) {
    ...
    if (sockfd < 0) {
        ...
        cout << "The ServerEE is up and running using UDP on port " + to_string(UDP_PORT) + "." << endl;
    }
    ...
    while (true) {
        ...
        if (numBytesRecvd < 0) {
            ...
            cout << "The ServerEE received a request from the Main Server." << endl;
        }
...
const string &serverEE::getCode() const {
    return code;
}

const string &serverEE::getCredits() const {
    return credits;
}

const string &serverEE::getProfessor() const {
    return professor;
}

const string &serverEE::getDays() const {
    return days;
}

const string &serverEE::getCourseName() const {
    return courseName;
}

void serverEE::setCode(const string &code) {
    serverEE::code = code;
}

void serverEE::setCredits(const string &credits) {
    serverEE::credits = credits;
}

void serverEE::setProfessor(const string &professor) {
    serverEE::professor = professor;
}

void serverEE::setDays(const string &days) {
    serverEE::days = days;
}

void serverEE::setCourseName(const string &courseName) {
    serverEE::courseName = courseName;
}
